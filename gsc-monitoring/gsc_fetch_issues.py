...
SITES = [
    ...
    "sc-domain:last-call.lourock.com",
...
def get_coverage_issues(service, site_url):
    try:
        response = (
            ...
            .index()
            .inspect(body={"inspectionUrl": site_url, "siteUrl": site_url})
            .execute()
        )
...
def main():
    ...
    try:
        ...
        verified_sites = [
            site["siteUrl"] for site in sites_response.get("siteEntry", [])
        ]
    ...
    output_file = (
        f"{RESULTS_DIR}/gsc_issues_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
